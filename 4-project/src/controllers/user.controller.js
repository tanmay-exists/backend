...
const registerUser = asyncHandler( async (req,res) => {
  ...
  const createdUser = await User.findById(user._id).select(
    "-password -refreshToken"  // password and refreshToken will be excluded
  )
  if(!createdUser){
...
const loginUser = asyncHandler(async (req, res) => {
  // req body-> data
  // username or email for loggingIn
  // find user
  // password check
  ...
  const options = {
    httpOnly: true,
    secure: true    // these two steps restricts the cookies to be modified by frontEnd, only server can modify them
  }
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
        fullName,
        email: email    // this line and the above line are just two ways of defining
      }
    },
    {
      new: true   // this helps in returning the user with updated fields
    }
  ).select("-password")
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        // _id: req.user._id won't work coz mongoose doesn't work here, aggregation pipelines code goes directly. Hence make object id of mongoose
